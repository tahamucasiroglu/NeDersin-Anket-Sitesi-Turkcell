...
namespace NeDersin.WepAPI.Controllers
{
    [Route("[controller]")]
    [ApiController]
    public class SurveyController : BasePostController< AddSurveyRequestDTO, UpdateSurveyRequestDTO, DeleteSurveyRequestDTO, GetSurveyResponseDTO>
    {
        private readonly ISurveyService surveyService;
        public SurveyController(ISurveyService surveyService, HateoasEnumeration hateoasEnumeration, ILogger<SurveyController> logger) : base(surveyService, hateoasEnumeration.GetSurveyControllerHateoas, logger)
        {
        ...
        [Authorize]
        public IActionResult GetSurveyById([FromBody]IdModel Id)
        {
            if (!ModelState.IsValid) return ModelStateNonValid<IdModel>(nameof(GetSurveyById));

            var result = surveyService.GetById(Id.Id);
