...
namespace NeDersin.WepAPI.Controllers
{
    ...
    [ApiController]
    [AllowAnonymous]
    public class ResponseAsyncController : BasePostController<AddResponseRequestDTO, UpdateResponseRequestDTO, DeleteResponseRequestDTO, GetResponseResponseDTO>
    {
        private readonly IResponseService responseService;
        public ResponseAsyncController(IResponseService responseService, HateoasEnumeration hateoasEnumeration, ILogger<ResponseController> logger) : base(responseService, hateoasEnumeration.GetResponseControllerHateoas, logger)
        {
        ...
        public async Task<IActionResult> GetById([FromBody] IdModel Id)
        {
            ...
            IReturnModel<GetResponseResponseDTO> result = await responseService.GetByIdAsync(Id.Id);
        ...
        public async Task<IActionResult> GetByQuestionId([FromBody] IdModel QuestionId)
        {
            ...
            IReturnModel<IEnumerable<GetResponseResponseDTO>> result = await responseService.GetByQuestionIdAsync(QuestionId.Id);
            LogResultError(result);
            return StaticHelperMethods.SolveResult(result, hateoasModel);
        }
        ...
        [HttpGet("[action]")]
        [ServiceFilter(typeof(LogFilterAttribute))]
        public async Task<IActionResult> GetByUserId([FromBody] IdModel UserId)
        {
            if (!ModelState.IsValid) return ModelStateNonValid<IdModel>(nameof(GetByUserId));
            IReturnModel<IEnumerable<GetResponseResponseDTO>> result = await responseService.GetByUserIdAsync(UserId.Id);
            LogResultError(result);
            return StaticHelperMethods.SolveResult(result, hateoasModel);
        }
    }
